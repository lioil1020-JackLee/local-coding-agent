...
class DiffServiceError(Exception):
    """Base exception for diff generation errors."""


class DiffApplyError(DiffServiceError):
    """Raised when a patch operation cannot be applied in memory."""
        ...
        if op.range.mode == PatchAnchorMode.LINE:
            ...
            if start_idx < 0 or end_idx > len(lines) or start_idx > end_idx:
                raise DiffApplyError(
                    f"replace_range line anchors out of bounds for {rel_path}: "
                    f"{op.range.start_line}-{op.range.end_line}"
                )
